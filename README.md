...
単なるCRUDアプリではなく、
- **DBを唯一の真実（Single Source of Truth）**
- **楽観的ロックによる同時編集防止**
- **Renderスリープへの耐障害設計**

など、実運用を想定した設計を採用しています。

---

## ● コンセプト

本アプリは **複数人が同時に手順書を操作する現場** を想定しています。
...
* **完了フラグ管理**: チェックを入れた瞬間にサーバー側で時刻を自動打刻（証跡管理）。
* **楽観ロックによる競合回避**: 同時に同じ手順を編集した場合、後に保存したユーザーにエラーを通知。
...
複数ユーザーが同時に編集した場合の  
**更新衝突（Lost Update）** を防ぐため  
`version` カラムを用いた **Optimistic Locking** を実装しています。
...
Render無料プランでは  
**15分アクセスがないとサーバーがスリープ**します。

そのためフロント側で

```
fetchTodos → 失敗 → 5秒後再試行
```
